...
const UserPanelAvatar = () => {
  ...
  const downloadAvatarHandler = async () => {
    ...
    try {
      const response = await fetch("http://localhost:4000/users/download-avatar", {
          method: "GET",
          headers: {
              "Authorization": `Bearer ${loggedInUser.token}`,
          },
  ...
  return (
    <div className="edit">
            ...
            <div className="edit__personal">
                <div className="row">
                    ...
                    <div className="col-12">
                        <label className="edit__label">عکس پروفایل جدید را انتخاب کنید : </label>
                        <input
        ...
        <div>
          <button className="edit__btn" onClick={downloadAvatarHandler}>دانلود عکس پروفایل فعلی</button>
        </div>

        <div>
          <button className="edit__btn" onClick={showAvatarHandler}>مشاهده عکس پروفایل فعلی</button>
        </div>

        <div>
          <button className="edit__btn" onClick={deleteAvatarHandler}>حذف عکس پروفایل فعلی</button>
        </div>

        {/* Modal for displaying the current avatar */}
        <Modal isOpen={isModalOpen} onClose={closeModal} imageUrl={currentAvatarUrl} />
